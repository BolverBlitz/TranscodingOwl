    ...
    constructor(encoders, encoders_config, log, reEncode) {
        ...
        internal_events.on('ffmpeg_status', (data) => {
            ...
            if (data.type === "time") {
                ...
                // Emitting progress to stdout for scriptv6.py
                console.log(`[OWL_PROGRESS] ${JSON.stringify({
                    file: this.jobs_list[data.task].name,
                    time: data.time,
                    total: bar.total,
                    percentage: bar.total > 0 ? ((data.time / bar.total) * 100).toFixed(2) : 0,
                    fps: data.fps || 0,
                    speed: data.speed || 0
                })}`);
        ...
        if (ffmpeg_result.code !== 0) {
            ...
            setTimeout(() => {
                try {
                    ...
                    fs.unlinkSync(task);
                    fs.renameSync(`${task.substring(0, task.lastIndexOf('.'))}.encoded.mkv`, `${task.substring(0, task.lastIndexOf('.'))}-TRCY.mkv`);
                } catch (e) {
