        ...
        child.stderr.on('data', (data) => {
            if (internal_events !== null) {
                ...
                if (time_match) {
                    ...
                    let extraData = {};
                    const fps_match = data.toString().match(fpsRegex);
                    if (fps_match) extraData.fps = parseInt(fps_match[1], 10);
                    
                    const speed_match = data.toString().match(speedRegex);
                    if (speed_match) extraData.speed = parseFloat(speed_match[1]);

                    internal_events.emit('ffmpeg_status', { task: command, type: "time", time: totalSeconds, ...extraData })
                }
            }
