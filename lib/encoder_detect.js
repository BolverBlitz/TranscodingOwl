...
const regexSpeedMatcher = /(?:speed=)(\d+(\.\d+)?x)(?![\s\S]*speed=\d+(\.\d+)?x)/;
...
const test_encoder = async () => {
    ...
    for (const [encoder] of Object.entries(encoders)) {
        const output = await executeCommand(`"${GlobffmpegPath}" -f lavfi -i color=size=1920x1080:rate=1:duration=600 -c:v ${encoder} -f null -`, "./")
        if (output.code === 0) {
            const match = output.hasSucceededMessage.match(regexSpeedMatcher);
            if (match !== null) {
                availableEncoders.push({ encoder, speed: match[1] });
            } else {
