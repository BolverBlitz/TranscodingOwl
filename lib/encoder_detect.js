...
const test_encoder = async () => {
    ...
    if(fs.existsSync(path.join(__dirname, '..', 'bin', 'availableEncoders.json'))) {
        try {
            const fileContent = fs.readFileSync(path.join(__dirname, '..', 'bin', 'availableEncoders.json'), 'utf-8');
            if (fileContent.trim() !== '') {
                readavaibleEncoders = JSON.parse(fileContent);
            }
        } catch (e) {
            console.warn("Failed to parse availableEncoders.json, starting fresh. Error:", e.message);
            readavaibleEncoders = [];
        }
    }
